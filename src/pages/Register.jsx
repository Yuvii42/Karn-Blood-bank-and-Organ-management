...
const Register = () => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      const parsedUser = JSON.parse(savedUser);
      setUserData(parsedUser);
      setQrCodeData(JSON.stringify(parsedUser)); // Set QR code data with full user details
    }
  }, []);
  ...
  // Handle registration and QR code generation
  const handleRegister = () => {
    localStorage.setItem("userData", JSON.stringify(userData)); // Save to localStorage
    setQrCodeData(JSON.stringify(userData)); // Set QR code data with full user details
  };
          ...
          <div className="mt-6 text-center">
            ...
            <div className="flex justify-center">
              <QRCodeCanvas
                value={qrCodeData}
                size={200}
                className="shadow-lg rounded-lg p-2 border border-gray-300"
              />
            </div>
